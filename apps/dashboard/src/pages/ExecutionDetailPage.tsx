    ...
    workflow: {
      ...
      configuration: JSON.stringify({
        fields: [
          ...
          { name: "total_amount", description: "Total amount", type: "currency" },
    ...
    files: [
      {
        ...
        extractionResult: JSON.stringify({
          fields: [
            { fieldName: "invoice_number", value: "INV-2024-001", confidence: 0.95 },
            { fieldName: "total_amount", value: 1250.00, confidence: 0.92 },
            { fieldName: "invoice_date", value: "2024-01-15", confidence: 0.88 },
          ],
          tables: [],
        }),
      },
  ...
  return (
    <div className="w-full px-4 py-8 space-y-8">
        ...
        <div>
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant={getStatusColor(mockExecution.status)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              <div>
                <p className="text-2xl font-bold">{formatDuration(mockExecution.startedAt, mockExecution.completedAt)}</p>
      ...
      <Card>
        ...
        <CardContent>
          <Tabs value={selectedFileIndex.toString()} onValueChange={(value) => setSelectedFileIndex(parseInt(value))}>
                      ...
                      <Card className="h-96 flex items-center justify-center">
                        <div className="text-center">
                          ...
                          <p className="text-sm text-muted-foreground max-w-sm">
                            {file.errorMessage || "An error occurred while processing this file"}
                          </p>
                        </div>
                      </Card>
                    ) : (
                            ...
                            <CardContent>
                              <div className="space-y-3">
                                {extractionResult.fields.map((field, fieldIndex) => {
                                  const fieldConfig = config.fields.find((f: any) => f.name === field.fieldName);
                                  return (
                                    <div key={fieldIndex} className="border-l-4 border-l-blue-500 bg-muted/50 rounded p-3">
                                      ...
                                      <div className="text-sm font-medium">
                                        {formatValue(field.value, fieldConfig?.type || 'text')}
                                      </div>
                                    </div>
                                  );
                                })}
                          ...
                          <Card>
                            ...
                            <CardContent>
                              {extractionResult.tables.map((table, tableIndex) => {
                                const tableConfig = config.tables.find((t: any) => t.name === table.tableName);
                                return (
                                    ...
                                    {table.rows.length > 0 ? (
                                      <div className="overflow-x-auto">
                                        <Table>
                                          <TableHeader>
                                            <TableRow>
                                              {tableConfig?.columns.map((column: any, colIndex: number) => (
