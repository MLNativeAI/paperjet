...
export default function ExecutionDetailPage() {
	...
	const successfulFiles = mockExecution.files.filter(
		(f) => f.status === "completed",
	).length;
	const failedFiles = mockExecution.files.filter(
		(f) => f.status === "failed",
	).length;
			...
			<div className="flex items-center justify-between">
				<div>
					<Button
						variant="ghost"
						onClick={() =>
							navigate({ to: `/workflows/${mockExecution.workflowId}/history` })
						}
						className="mb-4"
					>
					...
					<div className="flex items-center gap-4 mt-2">
						<span className="text-muted-foreground">
							{mockExecution.workflow.name}
						</span>
						<Badge variant="secondary">
							{mockExecution.workflow.documentType}
						</Badge>
						...
						<div className="space-y-4">
							<div>
								...
								<div className="flex items-center gap-2">
									...
									<Button
										variant="ghost"
										size="sm"
										onClick={() => copyToClipboard(mockExecution.id)}
									>
							...
							<div>
								...
								<div className="flex items-center gap-2">
									...
									<span>
										{new Date(mockExecution.startedAt).toLocaleString()}
									</span>
								</div>
							</div>
						</div>
