...
export function CategorizedExtractionResults({
  extractionResult,
}: CategorizedExtractionResultsProps) {
  const formatValue = (
  ...
  const fieldsByCategory =
    extractionResult.fields?.reduce(
      (acc, field) => {
        const category =
          field.category?.displayName || field.category || "Extracted Data";
        if (!acc[category]) {
          acc[category] = [];
        }
            ...
            <Card key={categoryName}>
              ...
              <CardContent>
                <div className="space-y-3">
                  {fieldsByCategory[categoryName].map(
