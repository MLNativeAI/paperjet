...
export function ExecutionResultsByCategory({ extractionResult, workflow }: ExecutionResultsByCategoryProps) {
  ...
  const getExtractedTable = (tableName: string): ExtractedTable | undefined => {
    return extractionResult.tables?.find((t) => t.slug === tableName);
  };
          ...
          {category.fields.length > 0 && (
            <div className="grid grid-cols-2 gap-4">
              {category.fields.map((field) => {
                const extractedValue = getExtractedValue(field.slug);
                    ...
                    <CardHeader>
                      <CardTitle>
                        {extractedValue?.value
                          ...
                          : toDisplayName(field.slug)}
                      </CardTitle>
                      <CardDescription>{toDisplayName(field.slug)}</CardDescription>
                    </CardHeader>
                    <CardContent>
                            ...
                            <TableHeader>
                              <TableRow>
                                {table.columns.map((col) => (
                                  <TableHead key={col.id}>{toDisplayName(col.slug)}</TableHead>
                                ))}
                              </TableRow>
                            </TableHeader>
                            <TableBody>
                              {extractedTable.rows.map((row, rowIndex) => (
                                <TableRow key={`${table.id}-row-${rowIndex}`}>
                                  {table.columns.map((col) => (
                                    <TableCell key={col.id}>{formatValue(row.values[col.slug], col.type)}</TableCell>
                                  ))}
                                </TableRow>
                              ))}
                            </TableBody>
