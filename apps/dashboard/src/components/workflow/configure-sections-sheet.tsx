...
export default function ConfigureSectionsSheet({ categories, isOpen, onClose, onSave }: ConfigureSectionsSheetProps) {
    ...
    const handleRenameSection = (categoryId: string, newName: string) => {
        setEditedCategories(editedCategories.map((cat) => (cat.categoryId === categoryId ? { ...cat, displayName: newName, slug: newName.toLowerCase().replace(/\s+/g, "_") } : cat)));
    };
            ...
            <SheetContent className="sm:max-w-[540px]">
                ...
                <div className="grid flex-1 auto-rows-min gap-4 px-4 py-4">
                    ...
                    <div className="grid gap-3">
                            ...
                            {editedCategories.map((category, index) => (
                                <div key={category.categoryId} className="flex items-center gap-2 p-2 border rounded-lg">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => handleMoveDown(index)} disabled={index === editedCategories.length - 1}>
                                            <GripVertical className="h-3 w-3" />
                                        </Button>
                                    </div>

                                    {/* Section Name Input */}
                                    <Input value={category.displayName} onChange={(e) => handleRenameSection(category.categoryId, e.target.value)} className="flex-1" placeholder="Section name" />

                                    {/* Remove Button */}
                                    <Button size="icon" variant="ghost" onClick={() => handleRemoveSection(category.categoryId)} className="h-8 w-8 text-destructive hover:text-destructive">
                        ...
                        {editedCategories.length === 0 && <div className="text-center py-8 text-muted-foreground">No sections yet. Click "Add Section" to create one.</div>}
                    </div>
                </div>
