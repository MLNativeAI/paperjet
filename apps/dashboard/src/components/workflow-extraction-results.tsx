...
export function WorkflowExtractionResults({ result }: WorkflowExtractionResultsProps) {
    ...
    try {
                                                    ...
                                                    {table.rows.map(
                                                        (
                                                            ...
                                                            <TableRow key={`row-${tableIndex}-${rowIndex}`}>
                                                                {Object.entries(row.values || row).map(([key, value]: [string, string | number | boolean | Date | null]) => (
        ...
        return <div className="p-3 text-sm text-red-600 bg-red-50 rounded">Error parsing results: {error instanceof Error ? error.message : "Unknown error"}</div>;
    }
}
