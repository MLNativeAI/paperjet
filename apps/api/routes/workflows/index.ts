...
async function analyzeWorkflowDocument(_fileId: string, workflowId: string, filename: string) {
    try {
...
const router = app
    .get("/", async (c) => {
        ...
        const workflows = await db.select().from(workflow).where(eq(workflow.ownerId, user.id));
    ...
    .put("/:id", async (c) => {
        try {
            ...
            if (!existingWorkflow || existingWorkflow.ownerId !== user.id) {
                return c.json({ error: "Workflow not found" }, 404);
            }
    ...
    .get("/:id/analysis-status", async (c) => {
        try {
            ...
            // Check if analysis is complete by looking at documentType and configuration
            const isAnalysisComplete = workflowData.documentType !== "Unknown";
...
Provide a structured analysis with practical, commonly needed information extraction focused on business processes.`,
    ...
    .post("/create-from-file", async (c) => {
        try {
            ...
            await db.insert(workflow).values({
                id: workflowId,
                name: workflowName,
                documentType: "Unknown", // Will be updated after analysis
                configuration: JSON.stringify({
                    fields: [],
                    tables: [],
                }),
            ...
            analyzeWorkflowDocument(fileId, workflowId, filename).catch((error) => {
    ...
    .post("/extract", async (c) => {
        try {
            ...
            const [fileRecord] = await db.select().from(file).where(eq(file.id, validatedData.fileId));
